...
async function getData(dominio) {
  //await new Promise((resolve) => setTimeout(resolve, 20000));

  try {
    const response = await fetch(
      /* process.env.NEXT_DATA= */ `https://test.digisoftware.online/api/organizations/by_url/?url=https://www.electronexus.com.ar`
    );
...
export async function generateMetadata() {
  ...
  return {
    ...
    icons: {
      icon:
        "https://test.digisoftware.online/" +
        data.organization.logo /* || "/favicon.ico" */,
    },
    openGraph: {
      title: data.organization.name,
      description: data.organization.description,
      images: "https://test.digisoftware.online/" + data.organization.logo,
    },
  };
}

export default async function page() {
  const data = await getData("dominio");
  const UrlBase = "https://test.digisoftware.online/";
